...
elder_behavior_system/
  core/
    filters.py          # Kalman Filter 기반 좌표 노이즈 필터링 스크립트
    send_alert.py       # 이상 행동 감지 시 Discord API 알림 전송 모듈

  database/
    db_manager.py       # SQLite3 데이터베이스 생성 및 데이터 CRUD 관리

  models/
    yolov8n-pose.pt     # 객체 및 포즈 추정을 위한 YOLOv8 가중치 파일

  ui/
    dashboard.py        # Streamlit 기반 실시간 모니터링 웹 대시보드

  .gitignore            # Git 추적 제외 설정 (DB, 로그, 캐시 등)
  README.md             # 프로젝트 개요 및 가이드 문서
  config.py             # 시스템 전역 설정 및 상수 관리
  extract_graph.py      # 칼만 필터 적용 전후 데이터 시각화 스크립트
  main.py               # 시스템 전체 프로세스 실행 메인 스크립트
```
