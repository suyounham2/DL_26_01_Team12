...
def main():
    ...
    while cap.isOpened():
            ...
            if len(keypoints) > 12:
                ...
                if left_shoulder[1] > 0 and right_shoulder[1] > 0:
                    ...
                    if prev_shoulder_y is not None:
                        ...
                        #STEP1: 가속도 판별
                        if y_velocity > VELOCITY_THRESHOLD and not is_falling:
                        ...
                        #STEP2: 각도, 지속 시간 판별
                        if is_falling:
                            if spine_angle < FALL_ANGLE_LIMIT and abs(y_velocity) < 0.02:
                                duration = time.time() - fall_start_time
                                if duration >= LIMIT_TIME:
                                    status_text = "FALL DETECTED"
                            else:
                                if spine_angle > RECOVERY_ANGLE:
                                    is_falling = False
                                    status_text = "NORMAL"
        ...
        if "FALL" in status_text:
            text_color = (0, 0, 255)  # 위험 상황: 빨간색 (BGR)
        else:
            text_color = (0, 255, 0)  # 정상 상황: 초록색 (BGR)
